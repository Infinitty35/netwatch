...
## DG05–DG09 — Context, contracts, targets, assurance and the one-shot run

Implemented locally on 20 September 2026 against `3fff5d9` (v0.32.0); not committed or
published.

**DG05 — network context.** `NetworkFingerprint` gained a `vpn` component, built from the
tunnel interfaces that are up, so bringing wg0 up is a different network and the live
sampler resets rather than comparing across it. Keys for hosts with no tunnel keep their
old spelling, so existing baselines still match. `Scope.processes_measured` separates a
host-wide fault, which really does affect every process, from an attribution that was
attempted and failed; the latter now reads "process not identified" instead of "all
processes".

**DG06 — rule contracts, tier one.** `Rule` carries `evidence`, `lookalikes`, `recovery`
and `platforms`. Eight rules — the ones implicated by the review — are filled in; the
other 22 declare themselves uncontracted rather than pretending otherwise.
`docs/diagnostic-coverage.md` is now generated by `netwatch diagnose coverage --doc`, and
a test fails when the committed copy and the catalogue disagree. The hand-written version
claimed 25 rules, omitted all five `target.*` entries and described five active rules as
unintegrated.

**DG07 — target address selection.** The probe tries every resolved address in order, up
to four, under a single 10-second budget, and records each attempt with its result. TLS
and HTTP run on the connection that actually succeeded. Previously only `addrs.first()`
was tried, so one dead address in a DNS answer reported a working service as down. The
`Host` header now carries the port when it is not the scheme's default and brackets an
IPv6 literal; without that the server answered about some other virtual host. SNI and the
effective endpoint are recorded.

**DG08 — assurance.** `tests/diagnose/corpus/` holds a deterministic recorded episode and
the decisions it must keep producing, and the replay test compares against that committed
file rather than against a recording it just made. `netwatch diagnose corpus` regenerates
it, which is the reviewed step for an intended semantic change. CI gained a
`diagnose-fault-lab` job running `tests/diagnose/fault_lab.py` under
`unshare --user --map-root-user --net`, with the JSON result kept as an artifact; the
script had been in the repo with nothing running it. Verified locally: 17 cases pass.

**DG09 — one bounded run.** `netwatch diagnose run [--target NAME] [--budget 30s]
[--format json]` samples for the budget and exits 0 for no finding, 1 for a finding, 2 for
incomplete and 3 for an error. Exit 2 exists so that a run which gathered nothing cannot
be read as a healthy host, and with `--target` only that target's own probes count as
evidence.

Validation: `cargo test --offline` passed **1192 tests**, 3 ignored; Clippy with
`-D warnings` and `cargo fmt --check` passed. The fault lab was run locally under an
isolated namespace and reported 17 passing cases.

Schema and consumer impact: `TargetObs` gained `attempts`, `effective_endpoint`, `sni`
and `http_authority`; `Scope` gained `processes_measured`; `NetworkFingerprint` gained
`vpn`; `IssueSpan` is now `Deserialize`. All new fields are `#[serde(default)]`.

Known remaining work: tier-two contracts for the other 22 rules, measurement provenance
on checks, and the field measurements behind the detection, precision and quiet-burden
gates. See [the plan](diagnose-professional-implementation-plan.md).

## DG00 — Four diagnostic corrections

Implemented locally on 20 September 2026 against `3fff5d9` (v0.32.0); not committed or
published. First package of
[the Diagnose plan](diagnose-professional-implementation-plan.md), whose packages are
numbered DG00–DG09 to keep them distinct from the PR01–PR16 sequence below.

- `GatewayObs.arp_ok` became `Option<bool>`. netwatch sends no ARP of its own, so it is
  `None` on every live host; it previously mirrored the ICMP result, which made the
  `arp_resolves` and `arp_fails` checks assert a probe that never ran. An unprivileged
  run, which cannot send ICMP either, printed "no arp reply from the gateway" every time.
- The `split_horizon` cause of `dns.hijack_suspect` had its check inverted: a private
  answer for a public name is the signature of an internal zone, and the code treated it
  as disproof, so the benign explanation scored 0.0 in exactly the case where it applies.
  It now supports the cause at the same weight as `interceptor`, since nothing netwatch
  measures separates a deliberate override from a hostile one.
- Added `VerifyOutcome::ClosedByOperator`. Any non-open issue previously decided as
  `Recovered`, so closing an issue by hand credited the preceding remediation step with a
  repair nothing measured. Only an auto-close, where the engine watched the verify
  condition hold, still reports `Recovered`.
- An inconclusive re-run now retracts the earlier answer for that test instead of being
  skipped. The superseded verdict used to go on ranking causes for up to its validity
  window, and the test was not offered again.

Validation: `cargo test --offline` passed **1171 tests**, 3 ignored; `cargo clippy
--offline --all-targets -- -D warnings`, `cargo fmt --check` and `git diff --check`
passed. Four regression tests added: `an_unprobed_arp_is_never_rendered_as_a_failed_arp`,
`a_private_answer_keeps_split_horizon_on_the_table`,
`closing_an_issue_by_hand_is_not_a_measured_recovery` (with
`an_auto_close_after_a_step_still_reports_a_measured_recovery` as its counterpart) and
`an_inconclusive_rerun_retracts_the_earlier_answer`. Three were checked against the
previous code and fail there; the ARP one could not be expressed before, as the field had
no unknown state.

`ml/schema.json` was regenerated: the split-horizon check id changed from
`public_name_public_answer` to `private_answer_for_a_public_name`, which changes the
feature name and the schema hash. Consumers pinned to the old hash need regenerating.

Known remaining work: `wrong_vlan_or_address_conflict` still reads Strong on a skipped
ARP check, because skipped checks leave the denominator entirely — DG02 owns that. The
serialised `VerifyOutcome` gains a variant, so older report readers may reject it.

## DG01–DG04 — Diagnostic credibility core

Implemented locally on 20 September 2026 against `3fff5d9` (v0.32.0); not committed or
published.

**DG01 — two claims the evidence could not support.** `TracerouteResult` now carries
`reached`: the native Linux path knows it from a port-unreachable, and the subprocess
path infers it when the final hop is the target address. `PathObs.destination_reached`
carries that into Diagnose. Hop blame now requires observed propagation; where every hop
after the lossy one is silent, the finding reports the loss and states that no hop can be
blamed. The evidence strings follow the branch that fired, instead of asserting
"loss propagates to later hops" in the branch where nothing answered.
`tcp.bufferbloat_remote` no longer names the receiver without the loaded/idle comparison:
absent it, the cause is `unlocalised_queueing` and the remediation is to run the test that
would place the queue.

**DG02 — evidence support replaces the pass ratio.** A check at weight 2.0 or above is a
discriminator; `Cause::confidence` caps a cause below Strong when one of those did not
run, and Strong otherwise requires either a complete set of checks or a passing
discriminator that can be named. `Cause::sufficient_evidence` and
`missing_discriminator` expose that, the Markdown report prints "Sufficient evidence: …"
or "Not measured: …", and the TUI appends "· needs <check>". Ranking sorts by confidence
before score, so a cause whose discriminator never ran cannot outrank a measured one. The
ARP checks on the gateway causes were given discriminating weight, since ARP is what
separates ICMP filtering from a layer-2 failure.

**DG03 — per-target observation identity.** `ObservationTimes.targets` became a map from
target name to completion time, and `sample_time` takes the issue's subject. A target on a
10-second interval can no longer confirm, hold or recover an issue about a target on a
5-minute one. Staleness is applied per target using its configured interval, carried on
`TargetObs.stale_after_secs`. Episodes record `target_ages` per name; the old single
`targets` age is kept for reading older recordings, which decode as no per-target
sampling rather than as evidence for whichever target is on screen. The export redactor
rewrites `target_ages` keys, which are map keys the value walk does not reach.

**DG04 — suppression follows dependency.** `Scope` gained `via_iface` and `via_resolver`,
populated for target findings from the lookup that answered and for gateway findings from
the primary interface. `scopes_overlap` became `depends_on`: findings measured on
different interfaces cannot explain each other, and a resolver failure only explains
targets that resolved through that resolver.

Validation: `cargo test --offline` passed **1181 tests**, 3 ignored; Clippy with
`-D warnings`, `cargo fmt --check` and `git diff --check` passed. Nine regression tests
added across the four packages. Each was checked against the previous code and fails
there, except the ARP one from DG00, which could not be expressed before.

Schema and consumer impact: `ml/schema.json` regenerated (new `destination_answered_the_trace`
check, renamed split-horizon check, new `unlocalised_queueing` cause).
`PathObs.destination_reached`, `TargetObs.stale_after_secs`, `Scope.via_iface`,
`Scope.via_resolver` and `ProbeAges.target_ages` are all `#[serde(default)]`, so older
recordings and reports still load. `VerifyOutcome::ClosedByOperator` is a new variant that
older readers will reject.

Known remaining work: DG05–DG09 in
[the plan](diagnose-professional-implementation-plan.md). Checks still carry no
measurement source, so two checks derived from one measurement can still both count as
support; that annotation belongs with the per-rule contracts in DG06.
