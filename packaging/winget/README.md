...
mkdir -p winget-pkgs/manifests/m/MattHartley/netwatch/0.33.0
cp packaging/winget/generated/* winget-pkgs/manifests/m/MattHartley/netwatch/0.33.0/

# 3. Validate (Windows only — needs winget itself)
winget validate --manifest manifests/m/MattHartley/netwatch/0.33.0
winget install --manifest manifests/m/MattHartley/netwatch/0.33.0

# 4. Open the PR
cd winget-pkgs && git switch -c netwatch-0.33.0
git add manifests/m/MattHartley/netwatch && git commit -m "New version: MattHartley.netwatch version 0.33.0"
git push -u origin netwatch-0.33.0 && gh pr create --repo microsoft/winget-pkgs --fill
```
...
`wingetcreate update MattHartley.netwatch --version 0.33.0 --urls <zip-url> --submit`.
